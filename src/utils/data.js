...
export const DataAPI = (data, root = {}) => {
  ...
  const getRandomName = (listId) => {
    ...
    const firstNames = uniq([
      ...get(list, "male.first", []),
      ...get(list, "any.first", []),
      ...get(list, "female.first", []),
    ]);
    const middleNames = uniq([
      ...get(list, "male.middle", []),
      ...get(list, "any.middle", []),
      ...get(list, "female.middle", []),
    ]);
    const lastNames = uniq([
      ...get(list, "male.last", []),
      ...get(list, "any.last", []),
      ...get(list, "female.last", []),
    ]);
    return `${getRandomItem(firstNames)}${
      middleNames.length ? " " + getRandomItem(middleNames) : ""
    }${lastNames.length ? " " + getRandomItem(lastNames) : ""}`;
  };
  ...
  const getRule = (ruleName, faction) => {
    ...
    return (
      (faction.rules || {})[ruleId] || {
        name: `${ruleId} not found`,
        id: ruleId,
      }
    );
  };
  ...
  const getWeapon = (weaponName, faction) => {
    ...
    return (
      faction.weapons[weaponId] || {
        id: `${weaponId} not found`,
        name: `${weaponId} not found`,
      }
    );
  };
  ...
  const getModel = (modelName, faction) => {
    ...
    return (
      (faction.models || {})[modelId] || {
        id: `${modelId} not found`,
        name: `${modelId} not found`,
      }
    );
  };
  ...
  const getModelList = (modelList, faction) => {
    const models = modelList
      .map((modelName) => getModel(modelName, faction))
      .filter((model) => !!model);
  ...
  const getPower = (powerName, faction) => {
    ...
    return (
      faction.powers[powerId] || {
        name: `${powerId} not found`,
        description: `Could not find power ${powerId}`,
      }
    );
  };
  ...
  const getStrategy = (strategyName, faction) => {
    ...
    return (
      faction.strategies[strategyId] || { name: `${strategyId} not found` }
    );
  };
  ...
  const getSubfactionNameList = (subfactionList, subfactions) => {
    ...
    const listString = subfactionList
      .map((subfactionKey) => subfactions[subfactionKey] || "")
      .filter((name) => !!name)
      .map((subfaction) => subfaction.name)
      .join(", ");
    return listString;
  };
  ...
  const getFaction = (factionName) => {
    ...
    const subfactionList = {
      ...get(faction, `subfactions`, {}),
      ...get(data, `subfactions`, {}),
    };
    return {
      ...faction,
      subfactions: subfactionList,
      powerCategories: {
        ...rootPowerCategories,
        ...globalPowerCategories,
        ...(faction.powerCategories || {}),
      },
      ...
      powers: {
        ...
        ...mapValues(faction.powers || {}, (strat) => ({
          ...strat,
          source: hasSubfactions(strat)
            ? `${data.name} - ${faction.name} - ${getSubfactionNameList(
                strat.subfactions,
                subfactionList
              )}`
            : `${faction.name}`,
      ...
      strategies: {
        ...
        ...mapValues(faction.strategies || {}, (strat) => ({
          ...strat,
          source: hasSubfactions(strat)
            ? `${faction.name} - ${getSubfactionNameList(
                strat.subfactions,
                subfactionList
              )}`
            : `${faction.name}`,
  ...
  const getFactionSubfactions = (factionName) => {
    return {
      ...get(data, `factions[${factionName}].subfactions`, {}),
      ...get(data, `subfactions`, {}),
    };
  };
  ...
  const getSubfaction = (factionName, subfactionName) => {
    return (
      get(
        data,
        `factions[${factionName}].subfactions[${subfactionName}]`,
        null
      ) || get(data, `subfactions[${subfactionName}]`, {})
    );
  };
  ...
  const getFactionWithSubfaction = (factionName, subfaction) => {
    ...
    const subfactionData =
      get(faction, `subfactions[${subfactionName}]`, null) ||
      get(data, `subfactions[${subfactionName}]`, {});
    const subfactionFilter = (thing) => {
      const subfactionset = new Set(get(thing, "subfactions", []));
      return (
        subfactionset.has(subfaction) ||
        subfactionset.size === 0 ||
        (subfactionset.has("none") && !subfactionData.restricted)
      );
    };
    ...
    const units = mapValues(
      pickBy(get(faction, "units", {}), subfactionFilter),
      (unit) => {
        return {
          ...unit,
    ...
    const dataWithSubfaction = {
      ...
      units: units,
      powers: mapValues(
        pickBy(get(faction, "powers", {}), subfactionFilter),
        (strat) => ({
          ...strat,
        })
      ),
      relics: mapValues(
        pickBy(get(faction, "relics", {}), subfactionFilter),
        (strat) => ({
          ...strat,
        })
      ),
      strategies: mapValues(
        pickBy(get(faction, "strategies", {}), subfactionFilter),
        (strat) => ({
          ...strat,
        })
      ),
    ...
    return getUnits(faction)
      .filter((unit) => {
        return keywords.length
          ? intersection(keywords, unit.keywords || ["Infantry"]).length
          : true;
      })
      .reduce((currModels, unit) => {
        return currModels.concat(
          getModels(unit, faction).map((model) => ({
            ...model,
  ...
  const getModelAddTerm = (modelLimit, model) => {
    let addTerm = "";
    const limitSplit =
      typeof modelLimit === "string" ? modelLimit.split("/") : null;
  ...
  const getAddLimitTermUnit = (addLimit) => {
    ...
    const limitSplit =
      typeof addLimit === "string" ? addLimit.split("/") : null;
    if (addLimit && addLimit > 1) {
  ...
  const getAdditionalModelsList = (unit, faction) => {
    ...
    (unit.models || []).forEach((model, modelIndex) => {
      ...
      if (min < max) {
        ...
        const addList = [
          {
            text: `${modelStats} for ${modelPoints}pts${
              upgradeCount > 1 ? ` each` : ""
            }`,
            points: modelPoints,
            modelIds: [modelIndex],
  ...
  const getOptionsList = (
    unit,
    faction,
    unitData = { selectedModels: {}, selectedModelsRaw: {} }
  ) => {
    ...
    if (unit.options) {
      (unit.options || []).forEach((option) => {
        if (option.addModel) {
          ...
          const isEach =
            modelLimit > 1 ||
            typeof modelLimit === "string" ||
            (model.max > 1 && !modelLimit);
          const addList = model.map((addedList) => {
            let totalCost = 0;
            const addedWeaponList = castArray(addedList);
            const addedListString = addedWeaponList
              .map((addedMod) => {
              ...
              .join(" and ");
            return {
              text: `${addedListString} for ${Math.trunc(totalCost)}pts${
                isEach ? " each" : ""
              }`,
              points: totalCost,
        ...
        model.options.forEach((option) => {
          if (option.replaceModel) {
            ...
            const isEach =
              replaceLimit > 1 ||
              modelReplaceLimit > 1 ||
              typeof modelReplaceLimit === "string" ||
              (model.max > 1 && !modelReplaceLimit);
            // Get current model cost
            const moddedModel = {
              ...model,
              rules: [...(model.rules || []), ...(unit.rules || [])],
            };
            totalOldCost += Math.trunc(
              getModelTotalPoints(moddedModel, faction, false)
            );
            const replaceTarget = castArray(option.replaceModel || []).map(
              (replacedWith) => {
                let totalCost = 0;
                const replacedWithList = castArray(replacedWith);
                const replaceList = replacedWithList
                  .map((addedMod) => {
                    const addedModel = getModel(addedMod, faction);
                    const rules = [
                      ...(unit.rules || []),
                    ...
                    const modelPoints = Math.trunc(
                      getModelTotalPoints(newModModel, faction, false)
                    );
                    totalCost += modelPoints;
                    const modelStats = formatModel(
                      addedModel,
                ...
                const limitSplit =
                  typeof modelReplaceLimit === "string"
                    ? modelReplaceLimit.split("/")
                    : null;
                if (limitSplit && limitSplit.length === 2) {
                  modelReplaceLimit = Math.floor(
                    (toNumber(limitSplit[0]) / toNumber(limitSplit[1])) *
                      numThisModelRaw
                  );
                }
                return {
                  text: `${replaceList} for ${totalCostDiff}pts${
                    isEach ? " each" : ""
                  }`,
                  ...
                  models: false,
                  limit:
                    ((modelReplaceLimit === "all" ? 1 : modelReplaceLimit) ||
                      numThisModelRaw) * (replaceLimit || 1),
                  choiceLimit:
                    ((modelReplaceLimit === "all" ? 1 : modelReplaceLimit) ||
                      numThisModelRaw) * (replaceLimit || 1),
                };
            ...
            upgrades.push({
              option: `Replace ${modelReplaceTerm} ${model.name} with${replaceTerm}`,
              list: sortBy(replaceTarget, "points"),
            });
          }
          if (option.replaceWeapon) {
            ...
            const replaceThingList = replaceWeaponList
              .map((replacedWeapon) => {
                totalOldCost += getWeaponCostForModel(
                  replacedWeapon,
                  model,
                  faction
                );
              ...
              .join(" and ");
            const replaceTarget = castArray(option.withWeapon || []).map(
              (replacedWith) => {
                let totalCost = 0;
                const replacedWithList = castArray(replacedWith);
                const replaceList = replacedWithList
                  .map((replaceWith) => {
                    totalCost += getWeaponCostForModel(
                      replaceWith,
                      model,
                      faction
                    );
            ...
            upgrades.push({
              option: `Replace ${modelReplaceTerm} ${model.name} ${
                replaceTerm === "all " || replaceLimit > 1 ? `models` : `model`
              }${
                count > 1 ? ` ${replaceTermCount}` : ""
              } ${replaceThingList} with${replaceTerm}`,
              list: sortBy(replaceTarget, "points"),
            });
          }
          if (option.replaceRule) {
            ...
            const replaceThingList = replaceWeaponList
              .map((replacedWeapon) => {
                ...
                totalOldCost += getRuleCostForModel(
                  replacedWeapon,
                  model,
                  faction
                );
                return `${
                  ruleData.inputs
                    ? `${ruleData.name}(${ruleData.inputs
                        .map((input) => replacedWeapon[input])
                        .join(", ")})`
                    : ruleData.name
                }`;
              })
              .join(" and ");
            const replaceTarget = castArray(option.withRule).map(
              (replacedWith) => {
                let totalCost = 0;
                const replacedWithList = castArray(replacedWith);
                const replaceList = replacedWithList
                  .map((replaceWith) => {
                    ...
                    const shortDescription = ruleData.description_short
                      ? ` (${ruleData.description_short})`
                      : "";
                    const ruleInputs = ruleData.inputs
                      ? `(${(ruleData.inputs || [])
                          .map((input) => replaceWith[input])
                          .join(", ")})`
                      : "";
                    const ruleDescription =
                      shortDescription || ruleInputs || "";
                    totalCost += getRuleCostForModel(
                      replacedWith,
                      model,
                      faction
                    );
            ...
            upgrades.push({
              option: `Replace ${replaceTerm}${model.name} ${
                replaceTerm === "all " || replaceTerm === "any "
                  ? "models"
                  : "model"
              }${
                count > 1 ? ` ${replaceTermCount}` : ""
              } ${replaceThingList} with`,
              list: sortBy(replaceTarget, "points"),
            });
          }
          if (option.addWeapon) {
            ...
            const addList = weapon.map((addedList) => {
              ...
              const limitSplit =
                typeof modelLimit === "string" ? modelLimit.split("/") : null;
              if (limitSplit && limitSplit.length === 2) {
                modelLimit = Math.floor(
                  (toNumber(limitSplit[0]) / toNumber(limitSplit[1])) *
                    numThisModel
                );
              }
              return {
                text: `${addedListString} for ${cost}pts${
                  isEach ? " each" : ""
                }`,
                points: cost,
                all: modelLimit === "all",
                ...
                limit:
                  ((modelLimit === "all" ? 1 : modelLimit) || numThisModel) *
                  (addLimit || 1) *
                  count,
                choiceLimit:
                  ((modelLimit === "all" ? 1 : modelLimit) || numThisModel) *
                  (addLimit || weapon.length) *
                  count,
              };
            });
            upgrades.push({
              option: `Upgrade ${modelAddTerm} ${model.name}${
                count > 1 ? ` ${replaceTermCount}` : ""
              } with${addTerm}`,
              list: sortBy(addList, "points"),
            });
          ...
          if (option.addRule) {
            ...
            const addList = weapon.map((addedList) => {
              ...
              const addedListString = addedWeaponList
                .map((added) => {
                  ...
                  totalCost += newCost;
                  const shortDescription = thing.description_short
                    ? ` (${thing.description_short})`
                    : "";
                  const ruleInputs = thing.inputs
                    ? `(${(thing.inputs || [])
                        .map((input) => added[input])
                        .join(", ")})`
                    : "";
                ...
                .join(" and ");
              const name = addedList.name
                ? `${addedList.name} (${addedListString})`
                : `${addedListString}`;
              const cost = Math.trunc(totalCost);
              const limitSplit =
                typeof modelLimit === "string" ? modelLimit.split("/") : null;
            ...
            upgrades.push({
              option: `Upgrade ${modelAddTerm} ${model.name} ${
                modelAddTerm === "all " || modelAddTerm === "any "
                  ? "models"
                  : "model"
              }${count > 1 ? ` ${replaceTermCount}` : ""} with${addTerm}`,
              list: sortBy(addList, "points"),
            });
          }
          if (option.removeRule) {
            ...
            upgrades.push({
              option: `Remove ${modelAddTerm} ${model.name} ${
                modelAddTerm === "all " || modelAddTerm === "any "
                  ? "models"
                  : "model"
              }${count > 1 ? ` ${replaceTermCount}` : ""} ${addTerm}`,
              list: sortBy(addList, "points"),
            });
          }
        });
      ...
      (unit.options || []).forEach((option) => {
        if (option.replaceWeapon) {
          ...
          const isEach =
            ...
            modelReplaceLimit > 1 ||
            typeof modelReplaceLimit === "string" ||
            (sum(unit.models.map((model) => model.max)) > 1 &&
              !modelReplaceLimit);
          const replaceThingList = replaceWeaponList
            .map((replacedWeapon) => {
              const avgCost = mean(
                getModels(unit, faction)
                  .filter((model) => model.shoot !== "-" && model.fight !== "-")
                  .map((model) => {
                    return getWeaponCostForModel(
                      replacedWeapon,
                      {
                        ...model,
          ...
          const replaceTarget = castArray(option.withWeapon || []).map(
            (replacedWith) => {
              ...
              return {
                ...
                limit:
                  (modelReplaceLimit === "all" ? 1 : modelReplaceLimit) ||
                  unitTotalModels * (replaceLimit || 1) * count,
                choiceLimit:
                  (modelReplaceLimit === "all" ? 1 : modelReplaceLimit) ||
                  unitTotalModels * (replaceLimit || 1) * count,
              };
          ...
          upgrades.push({
            option: `Replace ${modelReplaceTerm}${
              count > 1 ? ` ${replaceTermCount}` : ""
            } ${replaceThingList} with${replaceTerm}`,
            list: sortBy(replaceTarget, "points"),
          });
        ...
        if (option.replaceRule) {
          ...
          const replaceThingList = replaceWeaponList
            .map((replacedWeapon) => {
              const replace = replacedWeapon.id || replacedWeapon;
              const ruleData = getRule(replace, faction);
              const avgCost = Math.trunc(
                mean(
                  getModels(unit, faction).map((model) => {
                    return getRuleCostForModel(
                      replacedWeapon,
                      {
                        ...model,
                        rules: [...(unit.rules || []), ...(model.rules || [])],
        ...
        if (option.addWeapon) {
          ...
          const isEach =
            ...
            modelLimit > 1 ||
            typeof modelLimit === "string" ||
            (sum(unit.models.map((model) => model.max)) > 1 && !modelLimit);
          const weapon = castArray(option.addWeapon);
          const addList = weapon.map((addedList) => {
            let totalCost = 0;
            const addedWeaponList = castArray(addedList);
            ...
            return {
              text: `${addedListString} for ${totalCost}pts${
                isEach ? " each" : ""
              }`,
              points: totalCost,
              weapons: addedWeaponList,
              all: modelLimit === "all",
              limit:
                (modelLimit === "all" ? 1 : modelLimit) ||
                unitTotalModels * (addLimit || 1) * count,
              choiceLimit:
                (modelLimit === "all" ? 1 : modelLimit) ||
                unitTotalModels * (addLimit || 1) * count,
            };
        ...
        if (option.addRule) {
          ...
          const addList = weapon.map((addedList) => {
            ...
            return {
              text: `${name} for ${Math.trunc(totalCost)}pts${
                isEach ? " each" : ""
              }`,
  ...
  const getPowerCharge = (power, faction) => {
    ...
    const psychicCost = getRuleCostForModel(
      { id: "psychic" },
      dummyTarget,
      faction
    );
  ...
  const getWeaponCostForModel = (weapon, model, faction, modifiedWeapon) => {
    ...
    if (
      (weapon.mount && weapon?.mount?.length === 1) ||
      isString(weapon?.mount)
    ) {
      weaponMountMult = 0.8;
    }
  ...
  const getWeaponPoints = (wep, model, faction) => {
    ...
    const weaponPoints = sortBy(
      weps
        .map((weapon) => {
          let points = 0;
          const weaponModelMult =
            weapon.short === "Melee"
              ? (model.fight - CONSTANTS.baseStats.fight) * -1
              : (model.shoot - CONSTANTS.baseStats.shoot) * -1;
          const weaponAP = weapon.ap || 0;
          const weaponRules = weapon.rules || [];
          const range = weapon.short;
          const rangeCost =
            range === "Melee" ? (model.movement || 6) / 6 : range / 6;
          // const weaponDamage = weapon.damage || 1;
          points += weaponModelMult;
          // points += (weaponDamage - 1) * CONSTANTS.damageCost;
          const rangePoints =
            weapon.medium || range === "Melee" ? rangeCost : rangeCost / 2;
          points += rangePoints;
  ...
  const getWeaponList = (weaponList, faction) => {
    const weapons = weaponList
      .map((weaponName) => getWeapon(weaponName, faction))
      .filter((weapon) => !!weapon);
    return sortBy(weapons, "name");
  };

  const getRulesList = (ruleList, faction) => {
    const weapons = ruleList
      .map((ruleName) => getRule(ruleName, faction))
      .filter((rule) => !!rule);
    return sortBy(weapons, "name");
  };
  ...
  const getWeapons = (unit, faction) => {
    ...
    getModels(unit, faction).forEach((model) => {
      if (model.options) {
        model.options.forEach((option) => {
          const wepsArray = castArray(
            option.withWeapon || option.addWeapon || []
          );
          wepsArray.forEach((wepList) => {
            const wepListArr = castArray(wepList);
            wepListArr.forEach((wep) => {
    ...
    units.forEach((unit) => {
      getModels(unit, faction).forEach((model) => {
        ...
        if (model.options || unit.options) {
          [...get(unit, "options", []), ...get(model, "options", [])].forEach(
            (option) => {
              const wepArray = castArray(
                option.withWeapon || option.addWeapon || []
              ).flat();
    ...
    units.forEach((unit) => {
      ...
      getModels(unit, faction).forEach((model) => {
        ...
        if (model.options || unit.options) {
          [...get(unit, "options", []), ...get(model, "options", [])].forEach(
            (option) => {
              const addRule = castArray(
                option.addRule || option.withRule || []
              ).flat();
