...
export const formatModelCount = (model) => {
  console.log(model);
  const modelCount = !model.id ? `${model.min || 1}x ` : '';
  return `${modelCount}${model.name}`;
};

export const formatModelWeapons = (
  model,
  unit,
  faction,
  data,
  ...
  const equipString =
    model.weapons && model.weapons.length
      ? ` equipped with ${
          orderBy(model.weapons || [], weaponOrderer, ["asc"])
            .map((weapon) => {
      ...
      : "";
  const modelCount = hideCount || model.id ? "" : `${model.min || 1}x `;
  return `${modelCount}${model.name} ${equipString}`;
};

export const formatModelSpecial = (
  model,
  unit,
  faction,
  data,
...
export const formatModel = (model, unit, faction, data, options = {}) => {
  ...
  return `${modelCount}${model.name} ${equipString}${rulesString}`;
};
...
export const formatWeapon = (weapon, faction, data, options = {}) => {
  ...
  const wepName = weapon.name
    ? ` ${weapon.name} (${weaponData.name})`
    : weaponData.name;
  const wepMount = weaponMount
    ? ` [${weaponMount.join ? weaponMount.join(", ") : weaponMount}]`
    : "";
